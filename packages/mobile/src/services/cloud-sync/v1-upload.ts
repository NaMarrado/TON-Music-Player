...
export async function uploadMissingLocalToCloud(
  ...
  return scheduleMobileJob({
    ...
    run: async () => {
      ...
      const { manifest, localTracks, localArtworks } = await buildLocalManifest(
        config, onProgress, shouldCancel,
      );
      const remoteManifest = await readRemoteManifest(client, config);
      const targetsByKey = new Map<string, UploadTarget>();
      const addTarget = (target: UploadTarget): void => {
        if (!targetsByKey.has(target.key)) targetsByKey.set(target.key, target);
      };
      ...
      localArtworks.forEach(addTarget);
