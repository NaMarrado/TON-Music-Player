...
private struct TONIosDownloadActivityState {
...
@available(iOS 16.1, *)
private enum TONDownloadActivityRuntime {
  static let controller = TONDownloadActivityController()
}

@available(iOS 16.1, *)
private actor TONDownloadActivityController {
  ...
  func end(itemId: Int) async {
    ...
    for activity in activitiesById.values {
      await end(activity, contentState: finalState)
    }
  }

  func endActivities(except itemIds: Set<Int>) async {
    let systemItemIds = Activity<TONDownloadActivityAttributes>.activities.map {
      $0.attributes.itemId
    }
    let trackedItemIds = Array(activityByItemId.keys)
    let itemIdsToEnd = Set(systemItemIds + trackedItemIds).subtracting(itemIds)

    for itemId in itemIdsToEnd {
      await end(itemId: itemId)
    }
  }
  ...
  private func recoverActivity(
    ...
    if let trackedActivity = activityByItemId[itemId] {
      return trackedActivity
    }

    let matchingActivities = Activity<TONDownloadActivityAttributes>.activities.filter {
      $0.attributes.itemId == itemId
    }
    guard let recoveredActivity = matchingActivities.first else {
      return nil
    }

    activityByItemId[itemId] = recoveredActivity
    for duplicateActivity in matchingActivities.dropFirst() {
      await end(duplicateActivity, contentState: contentState)
    }
